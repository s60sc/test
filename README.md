...
<span style="color:red">**This is a complex app and some users are raising issues when the app reports an error, but this is the app notifying the user that there is an issue with their setup, which only the user can fix. Please only raise issues for actual bugs (unhandled library error or crash), or to suggest an improvement or enhancement. Thanks.**</span>
